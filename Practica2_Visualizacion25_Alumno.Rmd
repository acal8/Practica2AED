...
```{r}
# labs es muy útil. Desde labs podemos cambiar los nombres en la leyenda del gráfico
...
load(file = "data/weatherAUS.RData")

# Creamos el objeto ggplot pedido
library(ggplot2)
...
# Una forma de cambiar los parámetros por defecto es:
# grafico2 + scale_color_manual(values = c('blue', 'green'))

grafico3 <- grafico2 + ggtitle("Temperaturas máximas y mínimas")
...
# Parámetro shape para forma del punto

grafico2 <- ggplot(ds, aes(x = Temp9am, y = Pressure9am, shape = RainToday)) + geom_point()

# Si pongo geom_point(aes(RainToday)) si queremos que solo afecte a la capa de puntos
...
grafico4 <- grafico3 + labs(title = "Temperatura y presión a las 9 am",
       subtitle = "Coloreado según la lluvia",
       x = "Temperatura",
       y = "Presión")
...
Observamos que con el color se ve mejor la información que introduce la variable ``RainToday`` (con la forma es difícil de distinguir).
...
* Crea un objeto ggplot denominado ``iris.grafico1`` en el que esté definido el conjunto de datos ``iris_tidy`` alojado en el fichero ``iris.tidy.RData`` y un *aesthetics* en el que en el eje $x$ esté definida la variable ``Species`` y en el eje $y$ esté definida la variable ``Value``.
...
rm(list = ls())
library(ggplot2)
load(file = "data/iris_tidy.RData")
...
rm(list = ls())
library(ggplot2)
load(file = "data/Ejercicio4_Data.RData")
...
grafico2 <- ggplot(DatosEjer4, aes(x = Fecha, y = Ventas, color = Fabricante)) + geom_point() + geom_line()
grafico2

grafico3 <- ggplot(DatosEjer4, aes(x = Fecha, y = Ventas, color = Fabricante)) + geom_point() + geom_line(aes(linetype = Fabricante))
grafico3

grafico4 <- grafico3 + labs(title = "Ventas mensuales de diferentes compañías tecnológicas")
grafico4
...
rm(list = ls())
library(ggplot2)
load(file = "data/iceRiver.RData")
library(latex2exp)  # Para incluir código latex en una leyenda

# Tan solo la x es común para las dos series temporales. No propagar todo para las siguientes capas
grafico1 <- ggplot(ice.river.df, aes(x = time)) + geom_line(aes(y = flow.vat), color = "red") + geom_line(aes(y = flow.jok), color = "blue") +
  labs(title = "Caudales de los ríos islandeses Vatnsdakksa (azul) y Jokulsa Eystri (rojo)", x = 'Tiempo (años)', y = TeX('Caudal($m^3/s$)'))
# Lo que son variables siempre dentro de aes, las constantes fuera
grafico1
